...
parser.add_argument("--hpc", action="store_true", default=False, help="set to hpc mode")
parser.add_argument(
    "--data-path",
    type=str,
    default="/scratch/zc807/seq_gan/",
    metavar="PATH",
...
parser.add_argument(
    ...
    default=16,
    metavar="N",
    help="number of roll-outs (default: 16)",
)
parser.add_argument(
    "--vocab-size",
    type=int,
...
parser.add_argument(
    "--batch-size", type=int, default=64, metavar="N", help="batch size (default: 64)"
)
parser.add_argument(
    "--n-samples",
    type=int,
...
def train_generator_mle(
    gen, data_iter, criterion, optimizer, epochs, gen_pretrain_train_loss, args
):
    """
    Train a generator with MLE.
    """
...
def train_generator_pg(gen, dis, rollout, pg_loss, optimizer, epochs, args):
    """
    Train a generator with the guidance of policy gradients.
    """
    for epoch in range(epochs):
...
def train_discriminator(
    ...
    for epoch in range(epochs):
        ...
        print(
            "Epoch {}, train loss: {:.5f}, train acc: {:.3f}".format(
                epoch, avg_loss, acc
            )
        )
...
def adversarial_train(
    ...
    for i in range(args.g_steps):
        ...
        train_generator_pg(
            gen, dis, rollout, pg_loss, gen_optimizer, args.gk_epochs, args
        )
...
if __name__ == "__main__":
    ...
    for i in range(args.rounds):
        ...
        gen_loss = eval_generator(
            generator, gen_eval_iter, nll_loss, args
        )  # model arg changed from target_lstm to generator
        gen_adversarial_eval_loss.append(gen_loss)
        dis_loss, dis_acc = eval_discriminator(
            discriminator, dis_eval_iter, nll_loss, args
        )
        dis_adversarial_eval_loss.append(dis_loss)
        dis_adversarial_eval_acc.append(dis_acc)
        print(
            "Gen eval loss: {:.5f} \t Dis eval loss: {:.5f} \t Dis eval acc: {:.3f}\n".format(
                gen_loss, dis_loss, dis_acc
            )
        )
