...
class Discriminator(nn.Module):
    ...
    def __init__(
        ...
        self.convs = nn.ModuleList(
            [
                nn.Conv2d(1, num_f, (f_size, embedding_dim))
    ...
    def forward(self, x):
        """
        Args:
            x: A tensor of size (batch_size, seq_len)
        ...
        convs = [
            F.relu(conv(emb)).squeeze(3) for conv in self.convs
        ...
        out = transform * F.relu(highway) + (1.0 - transform) * out  # sets C = 1 - T
