...
class Multitrack(object):
    ...
    def check_validity(self):
        ...
        if not isinstance(self.tempo, np.ndarray):
            ...
            raise TypeError(
                "Data type of `tempo` must be of a subdtype of " "np.number"
            )
    ...
    def parse_pretty_midi(
        ...
        if algorithm == "custom":
            if not isinstance(first_beat_time, (int, float)):
                raise TypeError(
                    "`first_beat_time` must be a number when "
        ...
        self.tempo = np.tile(bpm, (1, 24)).reshape(
            -1,
        )
        ...
        for instrument in pm.instruments:
            ...
            note_ons = ((beat_indices + ratios) * self.beat_resolution).astype(int)
    ...
    def plot(
        ...
        if mode == "separate":
            ...
            if track_label != "none":
                patches = [
                    Patch(color=cmatrix[idx], label=get_track_label(track_label, track))
                    for idx, track in enumerate(self.tracks)
                ]
