...
def plot_pianoroll(
    ...
    if pianoroll.shape[1] != 128:
        raise ValueError("The shape of `pianoroll` must be (num_time_step, " "128)")
    ...
    if xtick is "beat" and beat_resolution is None:
        raise ValueError("`beat_resolution` must be a number when `xtick` " "is 'beat'")
...
def save_animation(
    ...
    **kwargs
    ...
    def make_frame(t):
        ...
        to_plot = pianoroll[start:end].T / 128.0
